...
class TestRedBlackSTDeletion(unittest.TestCase):
    ...
    def test_delete_minimum(self):
        ...
        tree = RedBlackST()
        for k in [10, 5, 15, 2]:
            def test_delete_all_string_keys(self):
                """Delete all string keys one by one and ensure the tree is empty at the end."""
                tree = RedBlackST()
                keys = ["d", "b", "f", "a", "c", "e", "g"]
                for k in keys:
                    tree.put(k, k.upper())
            ...
            def test_delete_with_string_keys_corrected(self):
                """Delete nodes when keys are strings, corrected to check size after all deletions."""
                tree = RedBlackST()
                for k in ["d", "b", "f", "a", "c", "e", "g"]:
                    tree.put(k, k.upper())
                tree.delete("b")
                ...
                for k in ["a", "c", "d", "e", "f", "g"]:
                    tree.delete(k)
                self.assertEqual(tree.size(), 0)
                self.assertIsNone(tree.root)

            def test_delete_left_child_no_keys_method(self):
                """Delete the left child and verify keys using in/not in, since keys() is not implemented."""
                tree = RedBlackST()
                tree.put(10, "root")
                tree.put(5, "left")
                tree.put(15, "right")
                ...
                self.assertIn(tree.root.key, [10, 15])
                self.assertNotIn(5, tree)
                # Instead of set(tree.keys()), check presence directly
                present_keys = [k for k in [10, 15] if k in tree]
                self.assertEqual(set(present_keys), {10, 15})

            def test_delete_right_child_no_keys_method(self):
                """Delete the right child and verify keys using in/not in, since keys() is not implemented."""
                tree = RedBlackST()
                tree.put(10, "root")
                tree.put(5, "left")
                tree.put(15, "right")
                ...
                self.assertIn(10, tree)
                self.assertIn(5, tree)
                self.assertNotIn(15, tree)
                present_keys = [k for k in [10, 5] if k in tree]
                self.assertEqual(set(present_keys), {10, 5})

            def test_delete_minimum_and_maximum_on_empty(self):
                """Deleting min and max from an empty tree should not raise."""
                tree = RedBlackST()
                try:
                    tree.delete(0)
                    tree.delete(100)
                except Exception as e:
                    self.fail(f"Deleting from empty tree raised an exception: {e}")
                self.assertTrue(tree.is_empty())
                self.assertEqual(tree.size(), 0)

            def test_delete_root_repeatedly(self):
                """Repeatedly delete the root and check the tree shrinks correctly."""
                tree = RedBlackST()
                for k in [10, 5, 15]:
                    tree.put(k, str(k))
                for _ in range(3):
